...
Level.prototype.obstacleAt = function (pos, size) {
  var xStart = Math.floor(pos.x); //4.999999 would be rounded to 4
  var xEnd = Math.ceil(pos.x + size.x); // 4.1 -> 5
  var yStart = Math.floor(pos.y);
  var yEnd = Math.ceil(pos.y + size.y);

  // Consider the sides, top and bottom of the level as walls
  ...
  for (var y = yStart; y < yEnd; y++) {
        for (var x = xStart; x < xEnd; x++)  {
          ...
          if (fieldType && fieldType != "floaterBg")
            return fieldType;
        }
    }
};
// Collission detection for the actors is handled seperately from tiles.
...
var maxStep = 0.05;
var wobbleSpeed2 = 10;
var wobbleDist2 = 0.15;

Mini.prototype.act = function(step) {
